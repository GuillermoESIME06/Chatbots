...
(function() {
    ...
    const config = {
        ...
        lottieTooltipTextsJson: getConfig(currentScript, 'lottieTooltipTextsJson', null, 'json'),          // data-lottie-tooltip-texts-json (array de textos)
    ...
    function createLottieTooltip() {
        ...
        const tooltipTexts = config.lottieTooltipTextsJson && Array.isArray(config.lottieTooltipTextsJson) && config.lottieTooltipTextsJson.length > 0
            ? config.lottieTooltipTextsJson
            : [config.lottieTooltipText];
        
        if (!config.lottieTooltipEnabled || tooltipTexts.length === 0 || !tooltipTexts[0]) return null;

        const el = document.createElement('div');
        el.id = 'flowise-lottie-tooltip-' + Date.now();
        el._tooltipTexts = tooltipTexts;
        el._currentIndex = 0;
        el.textContent = tooltipTexts[0];
        ...
        if (config.lottieTooltipEnabled) {
            ...
            if (lottieTooltipElement) {
                lottieButtonElement.addEventListener('mouseenter', () => {
                    ...
                    if (lottieTooltipElement._tooltipTexts && lottieTooltipElement._tooltipTexts.length > 1) {
                        lottieTooltipElement._currentIndex = (lottieTooltipElement._currentIndex + 1) % lottieTooltipElement._tooltipTexts.length;
                        lottieTooltipElement.textContent = lottieTooltipElement._tooltipTexts[lottieTooltipElement._currentIndex];
                    }
